...
<body>
    ...
    <div class="mainContainer">
        <h3>BBA FACULTY INFORMATIONS</h3>
        ...
        <div class="Container">                 
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'BBA' and Name ='Neaz Ahmed'" );
            oci_execute($stid);

            
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyOne">';
                echo '<img src="../images/bbb1.png" alt=""><br>';
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'BBA' and Name = 'DR. M. Anisha Khatun'");
            oci_execute($stid);

           
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyTwo">';
                echo '<img src="../images/bbb2.png" alt=""><br>';
                echo '<div class="details">';
                echo '<label for="">Name: ' . $row['NAME'] . '</label><br>';
                echo '<label for="">Email: ' . $row['EMAIL'] . '</label><br>';
                echo '<label for="">Address: ' . $row['ADDRESS'] . '</label><br>';
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'BBA' and Name = 'Syed Abul kalam Azad'");
            oci_execute($stid);

            
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyThree">';
                echo '<img src="../images/bbb3.png" alt=""><br>';
                echo '<div class="details">';
                echo '<label for="">Name: ' . $row['NAME'] . '</label><br>';
                echo '<label for="">Email: ' . $row['EMAIL'] . '</label><br>';
                echo '<label for="">Address: ' . $row['ADDRESS'] . '</label><br>';
