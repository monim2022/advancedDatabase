...
<body>
    ...
    <div class="mainContainer">
        <h3>LAW FACULTY INFORMATIONS</h3>
        ...
        <div class="Container">                 
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'LLB' and Name = 'Dr. Syeda Afroza'");
            oci_execute($stid);

            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyOne">';
                echo '<img src="../images/law1.png" alt=""><br>';
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'LLB' and Name = 'DR. Ferdousi Begum'");
            oci_execute($stid);

            
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyTwo">';
                echo '<img src="../images/law2.png" alt=""><br>';
                echo '<div class="details">';
                echo '<label for="">Name: ' . $row['NAME'] . '</label><br>';
                echo '<label for="">Email: ' . $row['EMAIL'] . '</label><br>';
                echo '<label for="">Address: ' . $row['ADDRESS'] . '</label><br>';
