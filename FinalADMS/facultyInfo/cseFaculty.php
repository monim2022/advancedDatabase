...
<body>
    ...
    <div class="mainContainer">
        <h3>CSE FACULTY INFORMATIONS</h3>
        ...
        <div class="Container">                 
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'Computer Science' and Name = 'Rezwan Ahmed'");
            oci_execute($stid);

            // Fetch faculty information and display it in HTML
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyOne">';
                echo '<img src="../images/reduanSir.png" alt=""><br>';
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'Computer Science' and Name = 'Sazzad Hossain'");
            oci_execute($stid);

            // Fetch faculty information and display it in HTML
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyTwo">';
                echo '<img src="../images/sazzadSir.jpg" alt=""><br>';
                echo '<div class="details">';
                echo '<label for="">Name: ' . $row['NAME'] . '</label><br>';
                echo '<label for="">Email: ' . $row['EMAIL'] . '</label><br>';
                echo '<label for="">Address: ' . $row['ADDRESS'] . '</label><br>';
			...
			$stid = oci_parse($conn, "SELECT NAME, EMAIL, ADDRESS, POSITION, DEPARTMENT FROM facultyInfo WHERE DEPARTMENT = 'Computer Science' and Name = 'S M Abdullah Shafi'");
            oci_execute($stid);

            // Fetch faculty information and display it in HTML
            while ($row = oci_fetch_assoc($stid)) {
                echo '<div class="facultyTwo">';
                echo '<img src="../images/safiSir.jpg" alt=""><br>';
                echo '<div class="details">';
                echo '<label for="">Name: ' . $row['NAME'] . '</label><br>';
                echo '<label for="">Email: ' . $row['EMAIL'] . '</label><br>';
                echo '<label for="">Address: ' . $row['ADDRESS'] . '</label><br>';
...
<?php
// Free statement and close connection outside of HTML structure
